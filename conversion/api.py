...
async def items_excluded_filter(avito_account: AvitoAccount, items: list):
    filtered_items = []
    excluded_items = await sync_to_async(list)(ExcludedItem.objects.filter(avito_account_id=avito_account.pk))
...
async def get_statistics_for_period(avito_account: AvitoAccount, period: str):
    ...
    action = f"/stats/v1/accounts/{avito_account.pk}/items"
    headers = {
    ...
    for chunk in item_chunks:
        ...
        response = avito_api.client.post(action, headers=headers, json=params, timeout=300)
