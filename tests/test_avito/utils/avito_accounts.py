...
def init_config_seller_data():
    if global_config.SELLER_AVITO_ACCOUNT_ID:
        avito_account = avitostat_test_api.get_avito_account(local_config.config.seller_avito_account_id)
        local_config.config.seller_access_token = avito_account.access_token
        local_config.config.seller_refresh_token = avito_account.refresh_token
        return

    avito_account = get_avito_account_info(
        access_token=local_config.config.seller_access_token,
        refresh_token=local_config.config.seller_refresh_token,
    )
...
def create_or_update_avito_account(created_by_user_id: int) -> tuple[avitostat_test_api.AvitoAccount, bool]:
    avito_account, created = avitostat_test_api.create_or_update_avito_account(
        access_token=local_config.config.seller_access_token,
        refresh_token=local_config.config.seller_refresh_token,
        telegram_id=str(local_config.config.summary_chat_id),
