...
from tests.test_avito import config as local_config
from tests.test_avito.utils import avito_api
from tests.test_avito.utils import avito_tokens
...
def get_chats(limit: int) -> list[ChatInfo]:
    ...
    action = f"/messenger/v2/accounts/{local_config.config.seller_avito_account_id}/chats"

    response = avito_api.get(avito_account_id=local_config.config.seller_avito_account_id, action=action, params={"limit": limit})
...
def get_chat_info(chat_id: str) -> ChatInfo | None:
    ...
    action = f"/messenger/v2/accounts/{local_config.config.seller_avito_account_id}/chats/{chat_id}"

    response = avito_api.get(local_config.config.seller_avito_account_id, action)
...
def get_messages(limit: int, offset: int = 0) -> list[AvitoMessage]:
    ...
    action = f"/messenger/v3/accounts/{local_config.config.seller_avito_account_id}/chats/{local_config.config.avito_chat_id}/messages/"
    ...
    response = avito_api.get(local_config.config.seller_avito_account_id, action, params=params)
    response.raise_for_status()

    if response.is_success:
        mark_chat_as_readed(local_config.config.seller_avito_account_id)
...
def send_message_from_seller_account(message: str):
    send_message(
        author_account_id=local_config.config.seller_avito_account_id,
        message=message,
    )
...
def send_message(author_account_id: int, message: str):
    ...
    action = f"/messenger/v1/accounts/{author_account_id}/chats/{local_config.config.avito_chat_id}/messages"
...
def mark_chat_as_readed(avito_account_id: int):
    ...
    action = f"/messenger/v1/accounts/{avito_account_id}/chats/{local_config.config.avito_chat_id}/read"
